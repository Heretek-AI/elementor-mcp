...
> **Current implementation (Heretek rebuild, this tree, ≥3.16) — the engine classes below are REAL, under `includes/migrate/`; prose elsewhere that names the earlier private-pro design (`Backup_Engine`, `EMCP_Tools_Migrate_HMAC`, a `Migrate_Targets`-style store, `Connector_Builder`) predates this rebuild and is superseded.** Transfer = `EMCP_Tools_Migration_Engine` (the four private helpers that lived in `EMCP_Tools_Migrate_Abilities` moved here verbatim: `push_packets`/`finalize`/`poll_job` + `push_archive` orchestration). Paired destinations = `EMCP_Tools_Migrate_Targets` (`{prefix}emcp_migrate_targets`, AES-encrypted secret, never returned plaintext; `redeem_pairing_code()` exchanges a single-use 15-min code from the destination connector's settings page over HTTPS). The connector is now **1.2.0**: adds `POST /pair/issue`, `POST /pair/exchange`, `GET /verify`, a minimal admin page that issues pairing codes + sets the option secret, and a `scope-restore` capability; its restore now treats a DB-less archive as "no DB" (not an error) and gates search-replace on `$did_db` AND an archived-table allowlist (`manifest.scope.db`) so files-only and selective-DB syncs behave. Sync = `EMCP_Tools_Sync_Engine` (`normalize_scope` → `{db:'all'|'none'|tables[], files:'all'|'none'|roots[]}`, `build_scoped_emcp`, `sync_to_target` with a 1.2.0 capability guard on non-full scopes); `EMCP_Tools_Packager::create_archive()` gained `db`/`file_roots`/`kind` opts and writes additive `scope`+`kind` manifest keys (no `scope` = full; `format_version` stays 2). **MCP surface:** the 4 migrate tools are unchanged plus a new **`url-search-replace`** tool (the old local search-replace body), disabled-by-default via `DEFAULTS_VERSION` 37; `migrate-site` accepts optional `target_id` (keeps raw `remote_url`+`secret_key` back-compat); `sync-to-live` is now the documented scoped push (`target_id`/raw + `scope{tables,file_roots}` + `confirm`). Restore on BOTH sides (`EMCP_Tools_Restore_Engine` and the connector) restricts SR to archived tables when the manifest is selective. Admin: the Backup & Migrate tab is now Archives | Push to Site | Sync sub-views (`EMCP_Tools_Migrate_Module` admin-post handlers `add_target`/`delete_target`/`verify_target`/`push`/`sync`; `assets/js/migrate.js`).
