...
* **Atomic Elements (Elementor 4.0+)**: `detect-elementor-version`, the universal `add-atomic-widget` / `update-atomic-widget` dispatchers (any atomic widget type by name), and the layout tools `add-flexbox` / `add-div-block`. Per-widget convenience types are reached through the dispatchers' widget catalog rather than one tool each.
...
* Added: the Backup & Migrate tab can now Restore an archive (manifest-hash verified), Download it, or Delete it (admin-only, nonce + capability-gated), with a restore-history panel.
...
* Added: four MCP tools so an AI agent can drive backup and migration (create-backup, list-backups — on by default — and migrate-site, sync-to-live — destructive, disabled by default, requiring confirm).
...
* Added: WooCommerce Pro integration, two dispatcher tools (woo-read / woo-write): read products/orders/customers + system status, and create/update/delete products + update orders. Reads on by default; writes off by default; delete operations require confirm:true. Registers only when WooCommerce is active. Coupons, refunds, reviews, shipping/tax, and gateway management are not yet exposed; WooCommerce widgets are placed through add-pro-widget.
